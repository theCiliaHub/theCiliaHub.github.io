...
    return [
        { gene: "IFT88", description: "Intraflagellar transport protein 88. Key component of the IFT-B complex.", localization: "Axoneme, Basal Body", ensembl_id: "ENSG00000032742", functional_summary: "Essential for intraflagellar transport and ciliary assembly." },
        { gene: "CEP290", description: "Centrosomal protein 290. Critical component of the ciliary transition zone.", localization: "Transition Zone", ensembl_id: "ENSG00000198707", omim_id: "610142", functional_summary: "Regulates ciliary gating and ciliopathy-related pathways." },
        { gene: "WDR31", description: "WD repeat domain 31. Involved in ciliary assembly and maintenance.", localization: "Axoneme", ensembl_id: "ENSG00000106459", functional_summary: "Required for proper ciliary structure and function." },
        { gene: "ARL13B", description: "ADP-ribosylation factor-like protein 13B. Involved in ciliary membrane biogenesis.", localization: "Ciliary Membrane", ensembl_id: "ENSG00000169379", functional_summary: "Critical for ciliary signaling and membrane trafficking." },
        { gene: "BBS1", description: "Bardet-Biedl syndrome 1 protein. Part of the BBSome complex.", localization: "Basal Body, Ciliary Membrane", ensembl_id: "ENSG00000166246", omim_id: "209901", functional_summary: "Involved in ciliary trafficking and BBSome assembly." },
        { gene: "ACE2", description: "Angiotensin-converting enzyme 2. Serves as the entry point for SARS-CoV-2.", localization: "Cilia", ensembl_id: "ENSG00000130234", omim_id: "300335", functional_summary: "Regulates blood pressure and acts as receptor for coronaviruses in respiratory cilia." }
    ];
}

function mapLocalizationToSVG(localization) {
    const mapping = { "ciliary membrane": ["ciliary-membrane", "axoneme"], "axoneme": ["ciliary-membrane", "axoneme"], "basal body": ["basal-body"], "transition zone": ["transition-zone"], "cilia": ["ciliary-membrane", "axoneme"], "flagella": ["ciliary-membrane", "axoneme"], "ciliary associated gene": ["ciliary-membrane", "axoneme"] };
    if (!localization) return [];
    return localization.split(',').flatMap(loc => { const trimmedLoc = loc.trim().toLowerCase(); return mapping[trimmedLoc] || []; }).filter(id => allPartIds.includes(id));
}
...
        }
        
        const filteredGenes = allGenes.filter(g => (g.gene && g.gene.toUpperCase().startsWith(query)) || (g.synonym && g.synonym.toUpperCase().includes(query))).slice(0, 10);
        
        if (filteredGenes.length > 0) {
            suggestionsContainer.innerHTML = '<ul>' + filteredGenes.map(g => `<li>${g.gene}${g.synonym ? ` (${g.synonym})` : ''}</li>`).join('') + '</ul>';
            
...
                        <div class="chart-container" style="position: relative; height:400px; width:100%;"><canvas id="localization-chart"></canvas></div>
                    </div>
                    <div id="functional-comparison" class="tab-content">
                        <div id="functional-cards-grid" class="functional-comparison-grid"></div>
                    </div>
                </div>
                <div id="comparison-placeholder" class="status-message"><p>Add genes to begin comparison.</p></div>
            </div>
        </div>`;
    
...
        const filteredGenes = allGenes.filter(g => g.gene.toLowerCase().includes(query) && !selectedCompareGenes.some(sg => sg.gene === g.gene)).slice(0, 10);
        
        if (filteredGenes.length > 0) {
...
    function renderTags() {
        tagsContainer.innerHTML = selectedCompareGenes.map(g => `<div class="gene-tag">${g.gene}<span class="remove-tag" data-gene="${g.gene}" title="Remove ${g.gene}">&times;</span></div>`).join('');
...
        container.innerHTML = selectedCompareGenes.map(g => `<div class="function-card"><h4>${g.gene}</h4><p>${g.functional_summary || 'No functional summary available.'}</p></div>`).join('');
    }
    
...
                    const term = loc.trim();
                    if (term) { localizationCounts[term] = (localizationCounts[term] || 0) + 1; }
                });
            }
        });
        
...
        if (localizationChartInstance) { localizationChartInstance.destroy(); }
        localizationChartInstance = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{ label: 'Number of Genes', data: data, backgroundColor: 'rgba(44, 90, 160, 0.7)', borderColor: 'rgba(44, 90, 160, 1)', borderWidth: 1 }]
            },
            options: {
...
                if (link.dataset.tab === 'visual-analysis') { renderLocalizationChart(); }
            });
        });
    }
}
...
        <p>Paste or upload your list of genes (e.g., from a differential expression analysis or a CRISPR screen) to perform two types of analysis:</p>
        <ul>
            <li><b>Ciliome Enrichment:</b> Calculates if your gene list is statistically enriched for known ciliary genes from the CiliaHub database.</li>
            <li><b>Localization Analysis:</b> Visualizes the subcellular localization patterns of the ciliary genes found within your list.</li>
        </ul>

        <textarea id="enrichment-genes-input"
            placeholder="Enter gene list here, one per line or separated by commas/spaces... e.g., TMEM17, IFT88, WDR31"
            style="width: 100%; min-height: 150px; padding: 1rem; border: 2px solid #e1ecf4; border-radius: 10px; font-size: 1rem; margin-top: 1rem; resize: vertical;"></textarea>
        
...
            <select id="download-format" style="display:none;">
...
            <h3 style="font-weight: bold; margin-bottom: 10px;">Plot Customization (For Localization & Matrix Plots)</h3>
...
                <div><label for="setting-font-family" style="display: block; margin-bottom: 5px;">Font Family</label><select id="setting-font-family"><option value="Arial">Arial</option><option value="Tahoma">Tahoma</option><option value="Times New Roman">Times New Roman</option><option value="Helvetica">Helvetica</option><option value="Verdana">Verdana</option></select></div>
                <div><label for="setting-font-size" style="display: block; margin-bottom: 5px;">Label Font Size</label><input type="number" id="setting-font-size" value="20" min="8" max="30" style="width: 60px;"></div>
                <div><label for="setting-font-weight" style="display: block; margin-bottom: 5px;">Label Weight</label><select id="setting-font-weight"><option value="normal">Normal</option><option value="bold" selected>Bold</option><option value="lighter">Lighter</option></select></div>
                <div><label for="setting-text-color" style="display: block; margin-bottom: 5px;">Text Color</label><input type="color" id="setting-text-color" value="#000000"></div>
                <div><label for="setting-axis-color" style="display: block; margin-bottom: 5px;">Axis Color</label><input type="color" id="setting-axis-color" value="#000000"></div>
                <div><label for="setting-y-axis-title" style="display: block; margin-bottom: 5px;">Y Axis Title</label><input type="text" id="setting-y-axis-title" value="Localization"></div>
                <div><label for="setting-enrichment-color1" style="display: block; margin-bottom: 5px;">Enrichment Color 1 (Low)</label><input type="color" id="setting-enrichment-color1" value="#edf8fb"></div>
                <div><label for="setting-enrichment-color2" style="display: block; margin-bottom: 5px;">Enrichment Color 2</label><input type="color" id="setting-enrichment-color2" value="#b2e2e2"></div>
                <div><label for="setting-enrichment-color3" style="display: block; margin-bottom: 5px;">Enrichment Color 3</label><input type="color" id="setting-enrichment-color3" value="#66c2a4"></div>
                <div><label for="setting-enrichment-color4" style="display: block; margin-bottom: 5px;">Enrichment Color 4</label><input type="color" id="setting-enrichment-color4" value="#2ca25f"></div>
                <div><label for="setting-enrichment-color5" style="display: block; margin-bottom: 5px;">Enrichment Color 5 (High)</label><input type="color" id="setting-enrichment-color5" value="#006d2c"></div>
            </div>
        </div>
...
    document.getElementById('download-plot-btn').addEventListener('click', () => {
        // Show format dropdown only when download is clicked
        const formatDropdown = document.getElementById('download-format');
        formatDropdown.style.display = 'inline-block';
        // A brief timeout to allow the user to select a format before triggering the download
        setTimeout(() => {
            downloadPlot();
...
                        bioRxiv preprint:
                        <a href="https://www.biorxiv.org/content/10.1101/2025.08.22.671678v1" target="_blank" rel="noopener noreferrer" aria-label="Open bioRxiv preprint in a new tab">https://www.biorxiv.org/content/10.1101/2025.08.22.671678v1</a>
                    </li>
                </ul>
...
            <div class="breadcrumb" style="margin-bottom: 2rem;"><a href="/" onclick="navigateTo(event, '/')" aria-label="Back to Home">← Back to Home</a></div>
...
                ${gene.reference ? `<div style="margin-top: 2rem; padding: 1rem; background: #f8f9fa; border-radius: 10px;"><strong>Reference:</strong> <a href="${gene.reference}" target="_blank" style="word-break: break-all;">${gene.reference}</a></div>` : ''}
            </div>
        </div>`;
    
...
// NOTE: All functions below this point are unchanged and are included for completeness.
...
                    ${gene.localization ? `<div class="gene-info"><strong>Localization:</strong> <span style="color: ${isSearchResult ? '#27ae60' : '#1e90ff'}; font-weight: 600;">${gene.localization}</span></div>` : ''}
...
                    ${gene.omim_id ? `<div class="gene-info"><strong>OMIM:</strong> <a href="https://www.omim.org/entry/${gene.omim_id}" target="_blank">${gene.omim_id}</a></div>` : ''}
                    ${gene.synonym ? `<div class="gene-info"><strong>Synonym:</strong> ${gene.synonym}</div>` : ''}
                    <div style="margin-top: 1rem; padding: 0.5rem; background: ${isSearchResult ? '#d5f4e6' : '#e8f4fd'}; border-radius: 5px; font-size: 0.9rem; color: ${isSearchResult ? '#27ae60' : '#1e90ff'};">Click to view detailed information →</div>
                `;
                
                entry.target.classList.add(isSearchResult ? 'search-result' : 'default');
                entry.target.onclick = () => navigateTo(null, `/${gene.gene}`);
...
    }, { rootMargin: '100px' });
    
    container.innerHTML = paginatedGenes.map(gene => `<div class="gene-card" data-gene='${JSON.stringify(gene)}'></div>`).join('');
    
...
    container.innerHTML = '';
    
    const defaultGenesButtons = defaultGenesNames.map(geneName => genesToDisplay.find(g => g.gene === geneName)).filter(Boolean);
        
    const searchGenes = searchResults.map(s => genesToDisplay.find(g => g.gene === s.gene)).filter(g => g && !defaultGenesNames.includes(g.gene));
        
...
        
        if (linkPath === path || (path.startsWith('/') && path !== '/' && path !== '/index.html' && linkPath === '/batch-query' && !['/download', '/contact', '/compare', '/expression', '/enrichment'].includes(path))) {
            link.classList.add('active');
        }
    });
}
...
            return localizations.includes(category.toLowerCase()) || (category === 'Cilia' && localizations.includes('ciliary membrane')) || (category === 'Flagella' && localizations.includes('axoneme')) || (category === 'Ciliary Associated Gene' && localizations.includes('ciliary associated gene'));
        }).length;
        return acc;
    }, {});
    
...
            scales: {
                y: { beginAtZero: true, title: { display: true, text: 'Number of Genes' }, ticks: { stepSize: 1 } },
                x: { title: { display: true, text: 'Localization' } }
            },
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: function(context) { return `${context.label}: ${context.raw} genes`; } } } }
        }
    });
}
...
    suggestionsDiv.innerHTML = suggestions.map(gene => `<div class="suggestion-item" onclick="selectExpressionGene('${gene}')"><div class="suggestion-gene">${gene}</div></div>`).join('');
...
                </tr>
            </thead>
            <tbody>${sortedTissues.map(([tissue, nTPM]) => `<tr><td style="padding: 0.8rem; border-bottom: 1px solid #e1ecf4;">${tissue}</td><td style="padding: 0.8rem; border-bottom: 1px solid #e1ecf4;">${nTPM.toFixed(2)}</td><td style="padding: 0.8rem; border-bottom: 1px solid #e1ecf4;">${getExpressionLevel(nTPM)}</td></tr>`).join('')}</tbody>
        </table>`;
...
                </tr>
            </thead>
            <tbody>${tissueExpressionData.slice(0, 50).map(item => `<tr style="cursor: pointer;" onclick="selectExpressionGene('${item.gene}')"><td style="padding: 0.8rem; border-bottom: 1px solid #e1ecf4;">${item.gene}</td><td style="padding: 0.8rem; border-bottom: 1px solid #e1ecf4;">${item.nTPM.toFixed(2)}</td></tr>`).join('')}</tbody>
        </table>`;
    tableWrapper.innerHTML = tableHTML;
}
...
                            <div style="text-align: center; padding: 2rem; color: #666;"><p>Loading human body visualization...</p></div>
                        </div>
                    </div>
...
                        </div>
                        <div id="expression-table-wrapper"><div style="text-align: center; padding: 2rem; color: #666; font-style: italic;">Click on an organ to see its gene expression data, or search for a specific gene</div></div>
                    </div>
                </div>
            </div>
        </div>`;
