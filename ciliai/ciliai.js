 ...
 * FINAL VERSION: Supports new 'Cloud' JSON format (Compressed Maps) + Switch Button
 */
window.renderUMAPPlot = async function(displayName, targetGenes = [], zoomToCellType = null) {
    ...
    // 2. Explicitly Purge Previous Plot
    // This prevents the "Kidney data drawing on top of Lung data" glitch
    try {
    ...
    // 4. Prepare Data
    // We use the full UMAP array (the 10k points generated by R)
    const sourceData = dataset.umap; 
    
    // Optional: Safety slice if data is unexpectedly large (e.g. >20k points)
    const maxPoints = 20000; 
    let renderData = sourceData;
    if (sourceData.length > maxPoints) {
        // Random subsample if too large
        renderData = [...sourceData].sort(() => 0.5 - Math.random()).slice(0, maxPoints);
    }

    const x = [], y = [], color = [], text = [], size = [];
    let maxExpr = 0;

    // 5. Get Expression Data Map
    // The new R script outputs expression as: { "Gene": { "CellType": Value, ... } }
    let geneExpressionMap = null;
    if (!isClusterView) {
        if (dataset.expression && dataset.expression[gene]) {
            // Direct lookup in dataset (Kidney new format)
            geneExpressionMap = dataset.expression[gene];
        } else if (window.CiliAI.cellDataCache && window.CiliAI.cellDataCache[gene]) {
            // Fallback to global cache (Lung format)
            geneExpressionMap = window.CiliAI.cellDataCache[gene];
        }
    }
    ...
    for (const point of renderData) {
        // Handle coordinates (New R script uses standard numbers, but safety check for arrays)
        const px = Array.isArray(point.x) ? point.x[0] : point.x;
        const py = Array.isArray(point.y) ? point.y[0] : point.y;
        
        // Handle Cell Type (Might be array in some versions)
        ...
        if (geneExpressionMap) {
            // Lookup value by Cell Type (Compressed format)
            // If cell type is missing in the map (sparse), it defaults to 0
            exprVal = geneExpressionMap[cellType] || 0;
        }
        ...
        if (isClusterView) {
            ...
            color.push(exprVal);
            size.push(exprVal > 0 ? 6 : 3); // Slightly larger for expressed cells
    ...
    const trace = {
        ...
        marker: { size: size, opacity: 0.8, line: { color: 'white', width: 0.2 } }
    };
    ...
    if (!isClusterView) {
        ...
        trace.marker.colorscale = dataset.colorScale || 
            (datasetKey === 'kidney' ? 
                [[0, '#EBF8FF'], [0.1, '#93C5FD'], [0.5, '#3B82F6'], [1, '#1E3A8A']] : // Blue for Kidney
                [[0, '#e2e8f0'], [0.1, '#fed7d7'], [1, '#c53030']] // Red for Lung
            );
        trace.marker.cmin = 0;
    ...
    // 7. Add UI Controls (Switch / Close)
    ...
    if (parent) {
        ...
        const nextLabel = nextDataset === 'lung' ? 'Lung' : 'Kidney'; // Simple label
        ...
        switchBtn.onclick = () => {
            ...
            // Recursively redraw with new dataset context
...
window.handleAIQuery = async function (query) {
    ...
    try {
        ...
        // Intent 10: Domains
            ...
            if (genes.length > 0) htmlResult = window.handleDomainQuery(genes);
        }
