 ...
 * • Loads ciliAI_master_database.json + ciliAI_lookups.json
 * • Lazy-loads phylogeny files only when required
 * • Fixes all routing bugs, fallback crashes, and missing function calls
 * ============================================================== */

// ==========================================================
// 0. GLOBAL DATA URL CONSTANT (MIGRATION FIX)
// ==========================================================
...
if (typeof window.drawDefaultUMAP !== "function") {
    window.drawDefaultUMAP = function () {
        window.log("drawDefaultUMAP() placeholder executed.");
        return "<p>Default UMAP plot placeholder.</p>";
    };
}

if (typeof window.drawDefaultTSNE !== "function") {
...
if (typeof window.startApplication !== "function") {
    window.startApplication = function () {
        window.log("Application started (startApplication placeholder executed).");
        // This is a placeholder for the main UI initialization call
...
window.CiliAI.cellDataCache = null;
...
// --- GLOBAL CONSTANTS FOR ORGANISM PANELS ---
    ...
    function getComplexPhylogenyTableMap() {
        return {
            ...
            "CENTRIOLE DISTAL APPENDAGES": ["CEP164", "CEP83", "SCLT1", "CEP89", "LRRC45", "CEP123", "ANKRD26", "FOPNL", "CEP128", "CEP135", "FBF1", "CCDC41", "CCDC120"],
...
// ==========================================================
// 2. CORE DATA LOADING (GCS Migration)
// ==========================================================

/**
 * CRITICAL NEW FUNCTION: Loads the core data files from GCS required for startup.
 * Sets the window.CiliAI.ready flag to true upon completion.
 */
async function loadCoreData() {
    window.updateStatus("Loading core CiliAI data from Google Cloud...", "loading");
    window.log("Starting core data fetch from GCS...");

    try {
        const [masterRes, lookupsRes, umapRes, cellRes] = await Promise.all([
            fetch(GCS_BASE_URL + 'ciliAI_master_database.json'),
            fetch(GCS_BASE_URL + 'ciliAI_lookups.json'),
            fetch(GCS_BASE_URL + 'umap_data.json'),
            fetch(GCS_BASE_URL + 'cellxgene_data.json'),
        ]);

        window.CiliAI.masterData = (await masterRes.json()).masterData;
        window.CiliAI.lookups = (await lookupsRes.json()).lookups;
        window.CiliAI_UMAP = await umapRes.json();
        
        // Assuming cellxgene_data.json has the key 'ciliary_gene_expression' as per structure
        const cellData = await cellRes.json();
        window.CiliAI.cellDataCache = cellData.ciliary_gene_expression || cellData; 

        // Set global state
        window.CiliAI.ready = true;
        window.updateStatus(`Data Loaded (${window.CiliAI.masterData.length} genes)`, "ready");
        window.log("Core CiliAI data loaded and ready.");
        
        // Final call to start the UI after data is ready
        window.startApplication();

    } catch (error) {
        console.error("CRITICAL: Failed to load core data from GCS:", error);
        window.updateStatus("CRITICAL ERROR: Failed to load core data.", "error");
        window.addChatMessage(`CRITICAL ERROR: Failed to load CiliAI data. Check GCS bucket access: ${error.message}`, false);
    }
}
    
/**
 * Loads the external data files required only by the Cilia Analysis Page plots.
 * Uses GCS_BASE_URL for consistency (assuming these files were also migrated).
 */
// ==========================================================
...
async function loadAnalysisData() {	
    // MIGRATION FIX: Use GCS_BASE_URL for consistent data source
    const analysisBaseUrl = GCS_BASE_URL; 
    try {
        window.log("Fetching specialized analysis data...");
        // NOTE: ciliahub_data.json and cilia_screens_data.json must also be in the GCS bucket
...
function handleHighLevelExplanations(query) {
    ...
    let result = handleProcessQuery(query); // Tries to build a step-by-step list
    if (result) return result;
...
// ==========================================================
// 5. SMART GRAPH QUERY — Multi-Hop Synthesis & Narratives (UPDATED)
// ==========================================================
/**
 * 5. SMART GRAPH QUERY — Multi-Hop Synthesis & Narratives
 * Handles: list retrieval, disease/complex simulated multi-hop synthesis
 * CRITICAL FIX: The logic has been streamlined to use the new helper functions 
 * and to correctly process the `intent` and `parsed` entities from the router.
 */
window.runGraphQuery = async function (query, intent, parsed) {
    try {
        window.log(`Executing runGraphQuery for intent: ${intent}`);
        
        const q = (query || "").toLowerCase();
        
        // 1. LIST RETRIEVAL INTENTS (Complex, Disease, Localization)
        if (parsed.intent === "complex_query" && parsed.complex) {
            const genes = window.getGenesByComplex(parsed.complex);
            return window.handleListQuery("complex", parsed.complex, genes);
        }
        
        // **User information applied:** Recognizing the distinction between general disease genes and specific organism
        // The list retrieval must use the human gene set derived from the masterData and diseaseMap
        if (parsed.intent === "disease_query" && parsed.disease) {
            const genes = window.getGenesByDisease(parsed.disease);
            return window.handleListQuery("disease", parsed.disease, genes);
        }

        // 2. DISEASE COMPARISON (Multi-Hop Synthesis)
        if (parsed.intent === "compare_diseases" && parsed.diseases && parsed.diseases.length === 2) {
            const A = parsed.diseases[0];
            const B = parsed.diseases[1];

            // Use the hardcoded synthesis for key examples, otherwise fall back to gene/complex overlap analysis
            if (normalizeTerm(A).includes('joubert') && normalizeTerm(B).includes('srtd') ||
                normalizeTerm(B).includes('joubert') && normalizeTerm(A).includes('srtd')) {
                // Use the rich hardcoded table from the previous step
...
| Overlap                    | CEP290, TMEM67            | Some shared genes             |`;
            ...
            // Generic overlap synthesis (same logic as the previously written stub, but now with defined helpers)
            const genesA = window.getGenesByDisease(A);
            const genesB = window.getGenesByDisease(B);
            const sharedGenes = genesA.filter(x => genesB.includes(x));
            
            let complexesA = (genesA || []).flatMap(g => window.getComplexesForGene(g) || []);
            let complexesB = (genesB || []).flatMap(g => window.getComplexesForGene(g) || []);
            const sharedComplexes = [...new Set(complexesA.filter(c => complexesB.includes(c)))];

            let html = `<div class="ai-result-card">### 🔬 **Comparative Analysis: ${A} vs ${B}**`;
            html += `<p>• **Genes Identified**: ${A}: ${genesA.length} — ${B}: ${genesB.length}</p>`;
            html += `<p>• **Shared Genes (${sharedGenes.length})**: ${sharedGenes.slice(0, 15).join(", ") || "(none)"}</p>`;
            html += `<p>• **Shared Complexes (${sharedComplexes.length})**: ${sharedComplexes.slice(0, 10).join(", ") || "(none)"} </p>`;
            html += `</div>`;
            return html;
        }

        // 3. MECHANISM SYNTHESIS (e.g., IFT-A mechanism)
        if (parsed.intent === "mechanism_synthesis" && parsed.complex === "IFT-A") {
             // Use the rich hardcoded synthesis
...
Mutations cause **retrograde transport failure** → accumulation of IFT-B and BBSome inside cilium → short/bulbous cilia → severe skeletal + renal phenotypes (Jeune, SRTD, CED). ${}`;
        ...
        // 4. LOCALIZATION QUERY (Placeholder list retrieval for localization)
        if (parsed.intent === "localization_query" && parsed.localization) {
            const loc = parsed.localization;
            // Filter master data based on localization field (assuming field is 'localization' and contains the term)
            const genes = window.CiliAI.masterData
                .filter(r => Array.isArray(r.localization) && r.localization.map(l => normalizeTerm(l)).includes(normalizeTerm(loc)))
                .map(r => r.Gene || r.gene);

            return window.handleListQuery("localization", loc, genes);
        }

        // 5. GENERIC CATCH-ALL
        return `<div class="ai-result-card">Graph query could not match a specific complex, disease, or comparison pattern. Please try again with a clearer term.</div>`;
    } catch (err) {
    ...
    try {
        ...
        if (compareDiseases) {
            ...
            if (shared.length) {
                ...
                // attempt complexes via masterData
                const complexesA = (genesA || []).flatMap(g => (window.getComplexesForGene ? window.getComplexesForGene(g) : []) || []);
                const complexesB = (genesB || []).flatMap(g => (window.getComplexesForGene ? window.get
