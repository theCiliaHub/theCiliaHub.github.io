...
window.fetchPhylogenySummary = fetchPhylogenySummary;
...
window.closePhyloModal = closePhyloModal;// ciliAI.js - Enhanced with phylogeny analysis, AI query handler, heatmap visualization, and robust autocomplete

// --- Global Data Cache ---

let ciliaHubDataCache = null;
let screenDataCache = null;
let phylogenySummaryCache = null;
...
window.displayCiliAIPage = async function displayCiliAIPage() {
    ...
    try {
        contentArea.innerHTML = `
                ...
                <div class="ciliai-main-content">
                    <div class="ai-query-section">
                        ...
                        <div class="ai-input-group autocomplete-wrapper">
                            <input type="text" id="aiQueryInput" class="ai-query-input" placeholder="e.g., genes for Joubert Syndrome, distribution of IFT88">
                        ...
                        <div class="example-queries">
                            <p><strong>Try asking:</strong> 
                                ...
                                <span>"distribution of IFT88"</span>,
                                <span>"genes in all species"</span>,
            ...
            <style>
                ...
                .phylo-viz-btn { margin-top: 1rem; padding: 0.6rem 1rem; background-color: #007bff; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 0.95rem; }
                .phylo-viz-btn:hover { background-color: #0056b3; }
                #phyloHeatmap { margin-top: 1.5rem; }
                .screen-table { width: 100%; border-collapse: collapse; margin-top: 1rem; }
                .screen-table th, .screen-table td { border: 1px solid #ddd; padding: 8px; text-align: left; }
                .screen-table th { background-color: #f2f2f2; font-weight: bold; }
                .evidence-section { margin-top: 1.5rem; }
                .evidence-toggle { margin-top: 1rem; padding: 0.6rem 1rem; background-color: #6c757d; color: white; border: none; border-radius: 6px; cursor: pointer; }
                .evidence-toggle:hover { background-color: #5a6268; }
                .evidence-content { display: none; margin-top: 1rem; padding: 1rem; background-color: #f8f9fa; border-left: 4px solid #6c757d; }
                .evidence-snippet { margin-bottom: 1rem; padding: 0.75rem; background-color: white; border-radius: 4px; }
                .evidence-snippet mark { background-color: #fff3cd; }
            </style>
        `;
    } catch (error) {
    ...
    await Promise.all([fetchCiliaData(), fetchScreenData(), fetchPhylogenySummary()]);
...
async function fetchPhylogenySummary() {
    if (phylogenySummaryCache) return phylogenySummaryCache;
    try {
        const resp = await fetch('https://raw.githubusercontent.com/theCiliaHub/theCiliaHub.github.io/refs/heads/main/phylogeny_summary.json');
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        phylogenySummaryCache = await resp.json();
        console.log("Loaded phylogeny summary:", Object.keys(phylogenySummaryCache).length, "genes");
        return phylogenySummaryCache;
    } catch (e) {
        console.error("Error fetching phylogeny summary:", e);
        return {};
    }
}
...
async function handleAIQuery() {
    ...
    const phylogenyData = await fetchPhylogenySummary();
    
    if (!data) {
        resultsContainer.innerHTML = `<p class="status-not-found">Error: CiliaHub data could not be loaded.</p>`;
        return;
    }
    ...
    try {
        // Handle phylogeny-specific queries
        if (lowerQuery.includes("distribution of") || lowerQuery.startsWith("where is")) {
            const geneMatch = query.match(/([A-Z0-9\-]+)/i);
            const gene = geneMatch ? geneMatch[1].toUpperCase() : null;
            
            if (gene && phylogenyData[gene]) {
                const info = phylogenyData[gene];
                title = `Phylogenetic Distribution of ${gene}`;
                resultHtml = `
                    <div class="result-card">
                        <h3>${title}</h3>
                        <p><strong>${gene}</strong> ${info.summary_text || "is found primarily in ciliated organisms."}</p>
                        ${info.present_in_all ? '<p style="color: #28a745;">✓ This gene appears in all organisms in the dataset.</p>' : ''}
                        <button class="phylo-viz-btn" onclick="window.renderPhylogenyHeatmap('${gene}')">Visualize Phylogeny</button>
                    </div>
                `;
            } else {
                resultHtml = `<div class="result-card"><h3>${title}</h3><p class="status-not-found">Sorry, I couldn't find phylogeny data for <strong>${gene || "that gene"}</strong>.</p></div>`;
            }
        }
        // Handle universal gene queries
        else if (lowerQuery.includes("all species") || lowerQuery.includes("universal") || lowerQuery.includes("conserved")) {
            const universal = Object.entries(phylogenyData)
                .filter(([_, v]) => v.present_in_all)
                .map(([gene]) => gene)
                .slice(0, 10);
            
            title = "Genes Present in All Species";
            if (universal.length > 0) {
                resultHtml = `
                    <div class="result-card">
                        <h3>${title}</h3>
                        <p><strong>${universal.join(", ")}</strong> appear in all organisms in the dataset.</p>
                        <p>These highly conserved genes likely play fundamental roles in ciliary structure and function.</p>
                    </div>
                `;
            } else {
                resultHtml = `<div class="result-card"><h3>${title}</h3><p class="status-not-found">No universal genes found in the dataset.</p></div>`;
            }
        }
        ...
        // Direct gene query
            ...
            resultHtml = `<div class="result-card"><p>I didn't understand that query. Try asking about:</p><ul><li>A disease (e.g., "genes for Joubert Syndrome")</li><li>A protein domain (e.g., "WD40 domain genes")</li><li>Gene localization (e.g., "transition zone localizing genes")</li><li>Phylogenetic distribution (e.g., "distribution of IFT88")</li><li>Universal genes (e.g., "genes in all species")</li></ul></div>`;
        }
...
window.renderPhylogenyHeatmap = async function(gene) {
    const phylogenyData = await fetchPhylogenySummary();
    const geneData = phylogenyData[gene];
    ...
    modal.style.display = 'block';
    modalBody.innerHTML = `<h3>${gene} Phylogenetic Distribution</h3><p>Loading visualization...</p>`;

    setTimeout(() => {
        if (!window.d3) {
            modalBody.innerHTML = `<h3>${gene} Phylogeny</h3><p class="status-not-found">D3.js library not loaded. Cannot render visualization.</p>`;
            return;
        }
        ...
        modalBody.innerHTML = `
            <h3>${gene} Phylogenetic Distribution</h3>
            <p>${geneData.summary_text || 'Distribution across species.'}</p>
            <svg id="phyloHeatmap"></svg>
        `;

        const svg = d3.select("#phyloHeatmap")
            .attr("width", width)
            .attr("height", height);

        const species = geneData.species.map(d => d.name);
        ...
        svg.selectAll("rect")
            ...
            .attr("height", 40)
            .attr("fill", d => color(d.present ? 1 : 0))
            .attr("stroke", "#333")
            .attr("stroke-width", 0.5)
            .append("title")
            .text(d => `${d.name}: ${d.present ? "Present" : "Absent"}`);

        svg.append("g")
            .attr("transform", `translate(0,${margin.top + 45})`)
        ...
        svg.append("text")
            ...
            .text(`${gene} Conservation Across Species`);
...
function setupAiQueryAutocomplete() {
    ...
    const exampleQueries = [
        ...
        "distribution of IFT88",
        "distribution of BBS1",
