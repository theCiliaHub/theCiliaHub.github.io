...
window.displayCiliAIPage = async function displayCiliAIPage() {
    ...
    try {
        contentArea.innerHTML = `
                    ...
                    <div class="ai-query-section">
                        ...
                        <div class="example-queries">
                             <p><strong>Try asking:</strong> 
                                ...
                                <span>"show phylogeny of IFT88"</span>.
                            </p>
                        </div>
                    </div>
            ...
            <div id="phyloModal" class="phylo-modal" style="display: none;">
                <div class="phylo-modal-content">
                    <span class="phylo-close" onclick="window.closePhyloModal()">&times;</span>
                    <div id="phyloModalBody"></div>
                </div>
            </div>
            ...
            <style>
                ...
                .phylo-modal { position: fixed; z-index: 2000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.6); }
                .phylo-modal-content { background-color: #fefefe; margin: 5% auto; padding: 2rem; border: 1px solid #888; border-radius: 12px; width: 85%; max-width: 900px; box-shadow: 0 4px 16px rgba(0,0,0,0.2); }
                .phylo-close { color: #aaa; float: right; font-size: 28px; font-weight: bold; cursor: pointer; line-height: 20px; }
                .phylo-close:hover, .phylo-close:focus { color: #000; }
...
async function fetchPhylogenyData() { if (phylogenyDataCache) return phylogenyDataCache; try { const response = await fetch('https://raw.githubusercontent.com/theCiliaHub/theCiliaHub.github.io/refs/heads/main/phylogeny_summary.json'); if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`); phylogenyDataCache = await response.json(); console.log('Phylogeny data loaded.'); return phylogenyDataCache; } catch (error) { console.error("Failed to fetch Phylogeny data:", error); return null; } }
...
async function handleAIQuery() {
    ...
    document.getElementById('plot-display-area').innerHTML = ''; // Clear previous plots
    let resultHtml = '';
    let match;
    ...
    try {
        if ((match = query.match(/(?:distribution of|phylogeny of)\s+([A-Z0-9\-]+)/i))) {
            ...
            renderPhylogenyHeatmap(geneSymbol); // Using the D3 heatmap function
            return; 
...
function formatSimpleResults(results, title) {
    ...
    results.forEach(gene => { html += `<li><strong>${gene.gene}</strong>: ${gene.description || 'No description available.'}</li>`; });
    return html + '</ul></div>';
}
...
window.renderPhylogenyHeatmap = async function(gene) {
    const phylogenyData = await fetchPhylogenyData();
    // Assuming your full phylogeny data (not just summary) is available.
    // This is a placeholder for where you would get the full species list for a gene.
    // For now, we'll create a mock detailed view from the summary.
    const geneSummary = phylogenyData[gene];
    const modal = document.getElementById('phyloModal');
    const modalBody = document.getElementById('phyloModalBody');

    if (!geneSummary) {
        modalBody.innerHTML = `<h3>${gene} Phylogeny</h3><p class="status-not-found">No detailed phylogeny data available for ${gene}.</p>`;
        modal.style.display = 'block';
        return;
    }

    modal.style.display = 'block';
    modalBody.innerHTML = `<h3>${gene} Phylogenetic Presence</h3><div id="phyloHeatmapViz"></div>`;

    if (!window.d3) {
        modalBody.innerHTML += '<p class="status-not-found">D3.js library not loaded.</p>';
        return;
    }

    // Mock data for visualization based on summary
    const vizData = [
        { group: "Ciliated", value: geneSummary.ciliated_prevalence },
        { group: "Non-Ciliated", value: geneSummary.non_ciliated_prevalence }
    ];

    const svg = d3.select("#phyloHeatmapViz").append("svg").attr("width", 400).attr("height", 200);
    ...
    const y = d3.scaleBand().range([0, height]).domain(vizData.map(d => d.group)).padding(0.1);
    g.append("g").call(d3.axisLeft(y));

    const x = d3.scaleLinear().domain([0, 1]).range([0, width]);
    g.append("g").attr("transform", `translate(0, ${height})`).call(d3.axisBottom(x).ticks(5).tickFormat(d3.format(".0%")));
...
window.closePhyloModal = function() {
    document.getElementById('phyloModal').style.display = 'none';
};
...
function setupAiQueryAutocomplete() {
    ...
    const exampleQueries = [ "phylogeny of IFT88", "distribution of ARL13B", "highly conserved genes", "show phylogeny of CEP290", "genes for Joubert Syndrome", "show me WD40 domain genes" ];
    aiQueryInput.addEventListener('input', () => {
        const inputText = aiQueryInput.value.toLowerCase();
        if (inputText.length < 2) { suggestionsContainer.style.display = 'none'; return; }
        const filteredSuggestions = exampleQueries.filter(q => q.toLowerCase().includes(inputText));
        if (filteredSuggestions.length > 0) { suggestionsContainer.innerHTML = filteredSuggestions.map(q => `<div class="suggestion-item">${q}</div>`).join(''); suggestionsContainer.style.display = 'block'; }
        else { suggestionsContainer.style.display = 'none'; }
    });
    suggestionsContainer.addEventListener('click', (e) => { if (e.target.classList.contains('suggestion-item')) { aiQueryInput.value = e.target.textContent; suggestionsContainer.style.display = 'none'; aiQueryInput.focus(); } });
    document.addEventListener('click', (e) => { if (!aiQueryInput.contains(e.target)) { suggestionsContainer.style.display = 'none'; } });
}
...
function setupGeneAutocomplete() {
    ...
    geneInput.addEventListener('input', async () => {
        ...
        if (currentTerm.length < 2) { suggestionsContainer.style.display = 'none'; return; }
        ...
        if (suggestions.length > 0) { suggestionsContainer.innerHTML = suggestions.map(gene => `<div class="suggestion-item">${gene}</div>`).join(''); suggestionsContainer.style.display = 'block'; }
        else { suggestionsContainer.style.display = 'none'; }
    });
    suggestionsContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('suggestion-item')) {
            ...
            if (lastChar && lastChar !== ',') terms.pop();
    ...
    document.addEventListener('click', (e) => { if (!geneInput.contains(e.target) && !suggestionsContainer.contains(e.target)) { suggestionsContainer.style.display = 'none'; } });
}
...
function setupCiliAIEventListeners() {
    ...
    if (!analyzeBtn || !aiQueryBtn || !visualizeBtn || !resultsContainer || !geneInput || !aiQueryInput) { console.warn('One or more CiliAI elements were not found.'); return; }
...
function analyzeGenesFromInput() {
    ...
    if (genes.length === 0) { const resultsContainer = document.getElementById('resultsContainer'); resultsContainer.innerHTML = '<p class="status-not-found">Please enter at least one gene symbol.</p>'; document.getElementById('resultsSection').style.display = 'block'; return; }
...
async function runAnalysis(geneList) {
    ...
    geneList.forEach(gene => { resultsContainer.insertAdjacentHTML('beforeend', createPlaceholderCard(gene, mode)); });
    ...
    for (const gene of geneList) {
        ...
        if (mode === 'expert' || mode === 'hybrid') { if (screenDataCache && screenDataCache[gene.toUpperCase()]) screenEvidence.push({ id: `screen-${gene}`, source: 'screen_data', context: renderScreenDataTable(gene, screenDataCache[gene.toUpperCase()]) }); }
        if (mode === 'nlp' || mode === 'hybrid') apiEvidence = await analyzeGeneViaAPI(gene, resultCard);
...
function createResultCard(gene, dbData, allEvidence, phyloData) {
    ...
    let summaryHtml = '';
    if (dbData && dbData.summary) { summaryHtml = `<div class="prediction-grid"><div class="prediction-box"><h4>Loss-of-Function (Length)</h4><p>${dbData.summary.lof_length}</p></div><div class="prediction-box"><h4>Percentage Ciliated</h4><p>${dbData.summary.percentage_ciliated}</p></div></div>`; }
    else { summaryHtml = '<p>No summary prediction available. Review literature and screen evidence for insights.</p>'; }
    let phylogenyHtml = '';
    if (phyloData) {
        let summaryText = '';
        const ciliatedPct = phyloData.ciliated_prevalence;
        ...
        phylogenyHtml = `<div class="phylogeny-section"><h4>Phylogenetic Profile</h4><p class="phylo-summary">${summaryText}</p><div class="phylo-grid"><div class="phylo-box">Conserved In<br><strong>${Math.round(ciliatedPct * 100)}%</strong><br>Ciliated Species</div><div class="phylo-box">Conserved In<br><strong>${Math.round(nonCiliatedPct * 100)}%</strong><br>Non-Ciliated Species</div></div></div>`;
    }
    let evidenceHtml = '';
    if (allEvidence.length > 0) {
        ...
        if (otherEvidence.length > 0) {
            const evidenceSnippets = otherEvidence.map(ev => `<div class="evidence-snippet">${ev.context.replace(new RegExp(`(${gene})`, 'ig'), `<mark>$1</mark>`)}<br><strong>Source: ${ev.source.toUpperCase()} (${ev.id})</strong></div>`).join('');
            evidenceHtml += `<button class="evidence-toggle" data-count="${otherEvidence.length}" data-content-id="evidence-${gene}">Show Other Evidence (${otherEvidence.length}) ▾</button><div class="evidence-content" id="evidence-${gene}">${evidenceSnippets}</div>`;
        }
        evidenceHtml += `</div>`;
    }
    return `<div class="result-card"><h3>${gene} - <span class="${statusClass}">${statusText}</span></h3>${summaryHtml}${phylogenyHtml}${evidenceHtml}</div>`;
}

async function analyzeGeneViaAPI(gene, resultCard) {
    const ESEARCH_URL = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi", ELINK_URL = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi", EFETCH_URL = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi";
    ...
    const geneRegex = new RegExp(`\\b${gene}\\b`, 'i'), sentSplitRegex = /(?<=[.!?])\s+/;
    let foundEvidence = [];
    const MAX_ARTICLES = 10, MAX_EVIDENCE = 5, RATE_LIMIT_DELAY = 350;
    ...
    try {
        ...
        for (const linkSet of linkSets) { const links = linkSet.linksetdbs?.find(set => set.dbto === 'pmc')?.links || []; pmcIds.push(...links); }
        ...
        if (pmcIds.length > 0) {
            ...
            if (fetchResp.ok) { const xmlText = await fetchResp.text(); const parser = new DOMParser(); const xmlDoc = parser.parseFromString(xmlText, "application/xml"); articles = Array.from(xmlDoc.getElementsByTagName('article')); }
        }
        if (articles.length === 0 && pmids.length > 0) {
            ...
            const fetchResp = await fetch(`${EFETCH_URL}?${fetchParams}`);
            if (fetchResp.ok) { const xmlText = await fetchResp.text(); const parser = new DOMParser(); const xmlDoc = parser.parseFromString(xmlText, "application/xml"); articles = Array.from(xmlDoc.getElementsByTagName('PubmedArticle')); }
        }

        for (const article of articles) {
            if (foundEvidence.length >= MAX_EVIDENCE) break;
            let pmid, textContent;
            if (article.tagName.toLowerCase() === 'article') { pmid = article.querySelector('article-id[pub-id-type="pmid"]')?.textContent || 'PMC'; const title = article.querySelector('article-title')?.textContent || ''; const body = Array.from(article.querySelectorAll('body p')).map(el => el.textContent).join(' '); textContent = `${title}. ${body}`; }
            else { pmid = article.querySelector('MedlineCitation > PMID')?.textContent || 'PubMed'; const title = article.querySelector('ArticleTitle')?.textContent || ''; const abstractText = Array.from(article.querySelectorAll('AbstractText')).map(el => el.textContent).join(' '); textContent = `${title}. ${abstractText}`; }
            if (!textContent || !geneRegex.test(textContent)) continue;
            const sentences = textContent.split(sentSplitRegex);
            for (const sent of sentences) { if (foundEvidence.length >= MAX_EVIDENCE) break; const sentLower = sent.toLowerCase(); if (geneRegex.test(sent) && [...LOCAL_ANALYSIS_KEYWORDS].some(kw => sentLower.includes(kw))) foundEvidence.push({ id: pmid, source: 'pubmed', context: sent.trim() }); }
        }
    } catch (error) { console.error(`Failed to fetch literature for ${gene}:`, error); const errorEl = resultCard?.querySelector('.status-searching'); if (errorEl) { errorEl.textContent = 'Lit Search Failed'; errorEl.className = 'status-not-found'; } }
...
function renderScreenDataTable(gene, screenInfo) {
    ...
    let screensObj = {};
    if (Array.isArray(screenInfo)) { screensObj = screenInfo.reduce((acc, entry) => { if (entry.source && entry.result) { acc[entry.source] = { hit: entry.result.toLowerCase() !== 'no effect', effect: entry.result, details: 'From raw data' }; } return acc; }, {}); }
    else if (screenInfo.screens) { screensObj = screenInfo.screens; }
    ...
    const tableHtml = `<table class="screen-table"><thead><tr><th>Screen</th><th>Hit?</th><th>Effect</th><th>Details</th></tr></thead><tbody>${screenKeys.map(key => { const d = screensObj[key] || { hit: false, effect: 'N/A', details: 'Not tested' }; const name = screenNames[key] || key; return `<tr><td>${name}</td><td>${d.hit ? '✅' : '❌'}</td><td>${d.effect}</td><td>${d.details}</td></tr>`; }).join('')}</tbody></table>`;
...
function renderScreenSummaryHeatmap(genes, screenData) {
    ...
    const numberScreens = { 'Kim et al. (2016) IMCD3 RNAi': 'Kim2016', 'Wheway2015': 'Wheway et al. (2015) RPE1 RNAi', 'Roosing2015': 'Roosing et al. (2015) hTERT-RPE1', 'Basu2023': 'Basu et al. (2023) MDCK CRISPR': 'Basu2023' };
    const signalingScreens = { 'Breslow et al. (2018) Hedgehog Signaling': 'Breslow2018' };
    const numberScreenOrder = Object.keys(numberScreens), signalingScreenOrder = Object.keys(signalingScreens);
    ...
    const geneLabels = genes.map(g => g.toUpperCase()), zDataNumber = [], textDataNumber = [], zDataSignaling = [], textDataSignaling = [];

    genes.forEach(gene => {
        const numberRowValues = [], numberRowText = [], signalingRowValues = [], signalingRowText = [];
        numberScreenOrder.forEach(screenName => { const screenKey = numberScreens[screenName]; let resultText = "Not in Screen"; if (screenData[gene]?.screens?.[screenKey]) resultText = screenData[gene].screens[screenKey].result || "Not Reported"; const mapping = numberCategoryMap[resultText] || numberCategoryMap["Not in Screen"]; numberRowValues.push(mapping.v); numberRowText.push(resultText); });
        signalingScreenOrder.forEach(screenName => { const screenKey = signalingScreens[screenName]; let resultText = "Not in Screen"; if (screenData[gene]?.screens?.[screenKey]) resultText = screenData[gene].screens[screenKey].result || "Not Reported"; const mapping = signalingCategoryMap[resultText] || signalingCategoryMap["Not in Screen"]; signalingRowValues.push(mapping.v); signalingRowText.push(resultText); });
        zDataNumber.push(numberRowValues); textDataNumber.push(numberRowText); zDataSignaling.push(signalingRowValues); textDataSignaling.push(signalingRowText);
    });
...
window.renderPhylogenyHeatmap = renderPhylogenyHeatmap;
window.closePhyloModal = closePhyloModal;
