...
function generateAnalysisPlots() {
    // Added 'gene-table-container' to the list of elements to hide on new analysis
    ['bubble-enrichment-container', 'matrix-plot-container', 'upset-plot-container', 'gene-table-container'].forEach(id => {
    ...
    // New function call to render the gene summary table
...
function renderGeneTable(foundGenes, notFoundGenes) {
    ...
    allGenes.forEach(gene => {
        ...
        tableHTML += `
            <tr>
                <td>${gene.name}</td>
                <td class="${statusClass}">${gene.status}</td>
            </tr>
        `;
    });
