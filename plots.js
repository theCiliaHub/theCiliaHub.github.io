...
function generateAnalysisPlots() {
    ['bubble-enrichment-container', 'matrix-plot-container', 'upset-plot-container', 'statistical-enrichment-container'].forEach(id => {
    ...
    } else if (selectedPlot === 'statistical') {
        renderStatisticalEnrichmentPlot(foundGenes);
    }
...
function downloadPlot() {
    ...
    if (selectedPlot === 'bubble') {
        ...
        fileName = 'CiliaHub_Statistical_Enrichment_Plot';
        const container = document.getElementById('statistical-enrichment-container');
        html2canvas(container, { backgroundColor: 'white', scale: 2 }).then(canvas => {
            if (format === 'png') {
                const a = document.createElement('a');
                a.href = canvas.toDataURL('image/png');
            ...
            console.error('Error downloading statistical enrichment plot:', error);
            alert('Failed to download the statistical enrichment plot.');
        });
    }
}
 ...
 */
function renderStatisticalEnrichmentPlot(foundGenes) {
    document.getElementById('statistical-enrichment-container').style.display = 'flex';
    if (window.statisticalPlotInstance) window.statisticalPlotInstance.destroy();
    ...
    if (significantResults.length === 0) {
        const plotWrapper = document.getElementById('statistical-enrichment-container');
    ...
    const legendContainer = document.getElementById('statistical-legend-container');
    if (legendContainer) {
    ...
    const ctx = document.getElementById('analysis-statistical-plot').getContext('2d');
    window.statisticalPlotInstance = new Chart(ctx, {
        type: 'bubble',
        data: { datasets: [dataset] },
        options: {
            responsive: true,
...
// =================================================================================
// ADDITIONAL UTILITY FUNCTIONS (Placeholder for potential missing code)
// =================================================================================
/*
 * The original plots.js has 836 lines, while this combined version has ~600 lines.
 * The missing ~236 lines may include helper functions like findGenes and sanitize,
 * additional event listeners, data initialization, or extensive comments.
 * Please provide the full original plots.js to identify and include any missing code.
 */
/*
...
function findGenes(geneNames) {
    ...
    geneNames.forEach(name => {
        const gene = window.CILIAHUB_DATA.genes.find(g => g.gene.toUpperCase() === name);
        if (gene) {
