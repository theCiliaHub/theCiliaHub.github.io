...
// =============================================================================
  ...
  function getPlotSettings() {
    return {
      ...
      upsetColor: document.getElementById('setting-upset-color')?.value || '#2c5aa0',
      matrixColor: document.getElementById('setting-bubble-color')?.value || '#377eb8'
    };
  }

function generateAnalysisPlots() {
  // Hide all plot containers and reset status
...
function renderBubbleMatrix(foundGenes) {
  ...
  const datasets = foundGenes.map((gene, index) => ({
    ...
    backgroundColor: settings.matrixColor
  }));
...
function renderUpsetPlot(foundGenes) {
  ...
  if (typeof UpsetJS !== 'undefined') {
    ...
    UpsetJS.render(wrapper, upset, {
      ...
      sets: { text: { family: settings.fontFamily, size: `${settings.fontSize}px`, weight: settings.fontWeight } },
      matrix: {
        set_labels: { text: { family: settings.fontFamily, size: `${settings.fontSize}px`, weight: settings.fontWeight } }
      },
      counts: {
        text: { family: settings.fontFamily, size: `${settings.fontSize}px` },
        bars: {
          ...UpsetJS.themes.default.counts.bars,
          background: settings.upsetColor,
          foreground: 'white'
        }
      }
    });
