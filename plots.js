...
        .explanation-section a:hover { text-decoration: underline; }
        ...
        /* Reverted to a more stable 2-column layout */
        .ciliaplot-main-layout {
            display: grid;
            grid-template-columns: 370px 1fr; /* Controls on left, plot on right */
            gap: 20px;
            align-items: start;
        }

        .controls-column .control-card { margin-bottom: 20px; }
...
        .plot-explanation { background-color: #f0f7ff; border: 1px solid #c8d9ed; border-radius: 5px; padding: 10px; margin-top: 10px; font-size: 0.85em; color: #333; line-height: 1.4; }
...
        #plot-display-area {
            position: relative; width: 100%; height: 75vh; border: 2px dashed #ccc;
            border-radius: 8px; display: flex; align-items: center; justify-content: center;
            color: #888; margin-top: 10px; overflow: hidden;
        }

        #plot-display-area > div, #plot-display-area > svg, #plot-display-area > canvas {
            width: 100% !important; height: 100% !important;
        }

        .gene-input-table-container { margin-top: 20px; }
...
const PLOT_CONFIG = {
    'localization_bubble': { label: 'Gene Localizations (Bubble)', group: 'Plotly Plots', explanation: 'Displays the subcellular localizations of your genes as bubbles.' },
    'functional_bar': { label: 'Functional Categories (Bar)', group: 'Plotly Plots', explanation: 'Shows the distribution of functional categories among your genes.' },
    'enrichment_bubble': { label: 'Enrichment Analysis (Bubble)', group: 'Plotly Plots', explanation: 'Analyzes functional category enrichment in your gene set.' },
    'balloon_plot': { label: 'Function vs Localization (Balloon)', group: 'Plotly Plots', explanation: 'Creates a heatmap showing the relationship between functional categories and subcellular localizations.' },
    'venn_diagram': { label: 'Gene Set Comparison (Venn)', group: 'Plotly Plots', explanation: 'Compares your gene list with the ciliary reference gene set.' },
    'network': { label: 'Complex Network (D3)', group: 'Advanced Plots', explanation: 'Visualizes protein-protein interactions and complex associations.' },
    'organelle_radar': { label: 'Organelle Radar (Chart.js)', group: 'Advanced Plots', explanation: 'Compares the organellar profile of your gene set against known markers.' },
    'organelle_umap': { label: 'Organelle UMAP (Chart.js)', group: 'Advanced Plots', explanation: 'Projects genes onto a 2D UMAP based on their organellar localization patterns.' },
    'screen_analysis': { label: 'Screen Analysis (Chart.js)', group: 'Advanced Plots', explanation: 'Displays results from genome-wide screens, showing how each gene affects ciliary function.' },
    'expression_heatmap': { label: 'Expression Heatmap (D3)', group: 'Advanced Plots', explanation: 'Shows tissue-specific expression patterns of your genes.' }
};
...
    let html = `<hr style="margin: 20px 0;"><div class="customization-grid">`;
...
// TODO: This placeholder MUST be replaced with the actual 2200+ Gold Standard gene list from the CiliaHub database.
const CILIAHUB_GOLD_STANDARD_GENES = new Set(['BBS1', 'BBS2', 'BBS4', 'BBS5', 'BBS7', 'CEP290', 'AHI1', 'NPHP1', 'ARL13B', 'IFT88']);
...
const organelleMarkerProfiles = {
    ...
    "Peroxisome": [0.3, 0.4, 0.6, 0.5, 0.4, 0.3, 0.7, 0.8], "Cilia": [0.05, 0.1, 0.15, 0.9, 0.95, 0.8, 0.3, 0.1]
};
...
            plotData.push({ x: localizations, y: Array(localizations.length).fill(gene.gene), mode: 'markers', type: 'scatter', name: gene.gene, marker: { size: custom.bubbleSize || 15, color: '#c8d9ed', line: { color: '#3f51b5', width: 1 } }, hoverinfo: 'x+y' });
        }
    });
    const layout = { title: { text: custom.title || 'Gene Subcellular Localizations', font: { size: custom.titleFontSize, family: custom.fontFamily }, y: 0.95 }, xaxis: { title: { text: 'Localization', font: custom.axisTitleFont }, visible: custom.showX, tickangle: -45, automargin: true }, yaxis: { title: { text: 'Gene', font: custom.axisTitleFont }, visible: custom.showY, automargin: true }, showlegend: false, height: 650, margin: { l: 120, r: 20, b: 150, t: 80 }, plot_bgcolor: 'white', paper_bgcolor: 'white' };
    Plotly.newPlot('plot-display-area', plotData, layout, { responsive: true });
}

function renderBarPlot(genes, custom) {
    const categoryCounts = new Map();
    genes.forEach(gene => { getCleanArray(gene, 'functional_category').forEach(cat => { categoryCounts.set(cat, (categoryCounts.get(cat) || 0) + 1); }); });
    const sorted = [...categoryCounts.entries()].sort((a, b) => b[1] - a[1]);
    const data = [{ x: sorted.map(e => e[1]), y: sorted.map(e => e[0]), type: 'bar', orientation: 'h', marker: { color: '#4CAF50' }, width: custom.barWidth || 0.8 }];
    const layout = { title: { text: custom.title || 'Functional Category Counts', font: { size: custom.titleFontSize, family: custom.fontFamily }, y: 0.95 }, xaxis: { title: { text: 'Number of Genes', font: custom.axisTitleFont }, visible: custom.showX, automargin: true }, yaxis: { title: { text: 'Category', font: custom.axisTitleFont }, visible: custom.showY, automargin: true }, height: 650, margin: { l: 300, r: 20, b: 80, t: 80 }, plot_bgcolor: 'white', paper_bgcolor: 'white' };
    Plotly.newPlot('plot-display-area', data, layout, { responsive: true });
}

function renderEnrichmentBubblePlot(genes, custom) {
    // This function remains largely the same, but benefits from the larger container and margin adjustments.
    const categoryCounts = new Map();
    genes.forEach(gene => { getCleanArray(gene, 'functional_category').forEach(cat => { categoryCounts.set(cat, (categoryCounts.get(cat) || 0) + 1); }); });
    const enrichmentData = [...categoryCounts.keys()].map(category => {
        const countInSet = categoryCounts.get(category);
        const enrichmentScore = (countInSet / genes.length) / 0.1; // Assuming 0.1 as base proportion
        const pValue = 1 / (enrichmentScore * 10);
        return { category, count: countInSet, enrichment: enrichmentScore, pValue };
    });
    const data = [{ x: enrichmentData.map(d => d.category), y: enrichmentData.map(d => d.enrichment), text: enrichmentData.map(d => `Category: ${d.category}<br>Count: ${d.count}<br>Enrichment: ${d.enrichment.toFixed(2)}<br>p-value: ${d.pValue.toFixed(4)}`), mode: 'markers', marker: { size: enrichmentData.map(d => (custom.bubbleSize || 15) * (d.count / 5 + 0.5)), color: enrichmentData.map(d => -Math.log10(d.pValue)), colorscale: 'Viridis', showscale: true, colorbar: { title: '-log10(p-value)', titleside: 'right' }, line: { color: '#000', width: 1 } } }];
    const layout = { title: { text: custom.title || 'Functional Category Enrichment', font: { size: custom.titleFontSize, family: custom.fontFamily }, y: 0.95 }, xaxis: { title: { text: 'Functional Category', font: custom.axisTitleFont }, visible: custom.showX, tickangle: -45, automargin: true }, yaxis: { title: { text: 'Enrichment Score', font: custom.axisTitleFont }, visible: custom.showY, automargin: true }, hovermode: 'closest', showlegend: false, height: 650, margin: { l: 120, r: 80, b: 160, t: 80 }, plot_bgcolor: 'white', paper_bgcolor: 'white' };
    Plotly.newPlot('plot-display-area', data, layout, { responsive: true });
}
...
        });
    });
    const data = [{ type: 'heatmap', x: localizations, y: functions, z: zData, colorscale: 'Blues', showscale: true }];
    const layout = { title: { text: custom.title || 'Function vs Localization', font: { size: custom.titleFontSize, family: custom.fontFamily }, y: 0.95 }, xaxis: { title: { text: 'Localization', font: custom.axisTitleFont }, visible: custom.showX, tickangle: -45, automargin: true }, yaxis: { title: { text: 'Functional Category', font: custom.axisTitleFont }, visible: custom.showY, automargin: true }, height: 700, margin: { l: 200, r: 50, b: 180, t: 80 }, plot_bgcolor: 'white', paper_bgcolor: 'white' };
    Plotly.newPlot('plot-display-area', data, layout, { responsive: true });
}
...
    clearAllPlots('plot-display-area');
    // **FIXED**: Using the full gold standard gene list for accurate comparison.
    const referenceCiliaryGenes = CILIAHUB_GOLD_STANDARD_GENES;
...
                    <div style="font-size: 24px; font-weight: bold;">${uniqueToUser.size}</div>
                </div>
...
                    <div style="font-size: 24px; font-weight: bold;">${uniqueToReference.size}</div>
                </div>
                <div style="position: absolute; left: 150px; top: 120px; width: 100px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold; color: #333; font-size: 24px;">${commonGenes.size}</div>
            </div>
            <div style="margin-top: 20px; font-size: 0.85em; color: #666;">Exact gene symbol matches only</div>
        </div>`;
}


// =============================================================================
// D3 & CHART.JS RENDERING FUNCTIONS
// =============================================================================
...
    const { nodes, links } = computeProteinComplexLinks(genes);
    console.log(`[Network Plot] Found ${nodes.length} nodes and ${links.length} links.`);
...
    const containerRect = container.getBoundingClientRect();
    if(containerRect.width < 50 || containerRect.height < 50) {
        container.innerHTML = '<p style="text-align: center; padding: 50px;">Error: Visualization container is too small to render network.</p>';
        return;
    }
...
    svg.append("text").attr("x", width / 2).attr("y", 20).attr("text-anchor", "middle").style("font-size", "16px").style("font-weight", "bold").text(custom.title || "Protein Complex Network");
    
...
    
    const link = svg.append("g").selectAll("line").data(links).enter().append("line").style("stroke", "#999").style("stroke-opacity", 0.8).style("stroke-width", 2);
...
    
    nodeGroup.append("circle").attr("r", 12).style("fill", "#3498db").style("stroke", "#2980b9").style("stroke-width", 2);
    nodeGroup.append("text").text(d => d.id).attr("x", 15).attr("y", 5).style("font-family", custom.fontFamily || "Arial").style("font-size", "11px").style("fill", "#333");
    
    simulation.on("tick", () => {
...
                matchedProfileKey = 'Cilia';
            } else {
                matchedProfileKey = Object.keys(organelleMarkerProfiles).find(key => key !== 'Cilia' && loc.toLowerCase().includes(key.toLowerCase()));
            }
            if (matchedProfileKey) {
                organelleMarkerProfiles[matchedProfileKey].forEach((val, i) => userProfile[i] += val);
                geneAddedToProfile = true;
            }
        });
        if (geneAddedToProfile) contributingGenes++;
    });
    
...
        label: name, data, borderColor: d3.schemeTableau10[i % 10], backgroundColor: d3.schemeTableau10[i % 10] + '20', borderWidth: 2,
        hidden: false, // **FIXED**: All organelles are visible by default.
        pointRadius: 3
    }));
    datasets.push({ label: 'Your Gene Set', data: userProfile, borderColor: '#e74c3c', backgroundColor: '#e74c3c20', borderWidth: 3, pointRadius: 4, hidden: false });
    
    currentPlotInstance = new Chart(ctx, { type: 'radar', data: { labels: fractionLabels, datasets: datasets },
        options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: custom.title || "Organellar Profile Comparison", font: { size: custom.titleFontSize || 16 } }, legend: { position: 'bottom', labels: { usePointStyle: true } } }, scales: { r: { beginAtZero: true, max: 1.0, ticks: { stepSize: 0.2 } } } }
    });
}
...
    const ctx = container.querySelector('canvas').getContext('2d');
    
    // **FIXED**: All background datasets are visible by default.
    const backgroundDatasets = Object.entries(precomputedUMAP).map(([name, data], i) => ({
        label: name, data: data, backgroundColor: d3.schemeCategory10[i % 10] + '77', pointRadius: 4, pointHoverRadius: 6
    }));
    
    const userGeneData = [];
...
        const ciliaTerms = ['basal body', 'transition zone', 'cilia', 'cilium', 'ciliary'];
        if (localizations.some(loc => ciliaTerms.some(term => loc.toLowerCase().includes(term)))) {
            foundOrganelle = 'Cilia';
        } else {
            foundOrganelle = Object.keys(precomputedUMAP).find(org => org !== 'Cilia' && localizations.some(loc => loc.toLowerCase().includes(org.toLowerCase())));
        }
        if (foundOrganelle) {
            const basePoint = precomputedUMAP[foundOrganelle][i % precomputedUMAP[foundOrganelle].length];
            userGeneData.push({ ...basePoint, gene: gene.gene, organelle: foundOrganelle });
        }
    });
    
    const userDataset = { label: 'Your Genes', data: userGeneData, backgroundColor: '#e74c3c', borderColor: '#c0392b', pointRadius: 8, pointHoverRadius: 10, borderWidth: 2 };
    
    currentPlotInstance = new Chart(ctx, { type: 'scatter', data: { datasets: [...backgroundDatasets, userDataset] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: custom.title || "Organellar UMAP Projection", font: { size: custom.titleFontSize || 16 } }, legend: { position: 'bottom', labels: { usePointStyle: true } }, tooltip: { callbacks: { label: function(context) { return context.dataset.label === 'Your Genes' ? `${context.raw.gene} (${context.raw.organelle})` : context.dataset.label; } } } }, scales: { x: { display: true, title: { display: true, text: 'UMAP 1' } }, y: { display: true, title: { display: true, text: 'UMAP 2' } } } }
    });
}

function renderGeneScreenAnalysis(genes, container, custom) {
    // This function remains largely the same, but benefits from layout changes
    clearAllPlots(container.id);
    container.innerHTML = `<canvas></canvas>`;
    const ctx = container.querySelector('canvas').getContext('2d');
    const processedData = [];
    const geneIndexMap = {};
    genes.forEach(gene => { if(!geneIndexMap[gene.gene]) geneIndexMap[gene.gene] = Object.keys(geneIndexMap).length; });
    const geneLabels = Object.keys(geneIndexMap).sort((a, b) => geneIndexMap[a] - geneIndexMap[b]);

    genes.forEach(gene => {
        if (!gene.screens) return;
    ...
    const datasets = Object.keys(groupedData).map(c => ({ label: c, data: groupedData[c], backgroundColor: classificationColors[c] || "#95A5A6" }));

    currentPlotInstance = new Chart(ctx, { type: 'scatter', data: { datasets },
        options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: custom.title || 'Gene Screen Analysis', font: { size: custom.titleFontSize || 16 } }, legend: { position: 'bottom' }, tooltip: { callbacks: { label: ctx => `${ctx.raw.gene}: ${ctx.raw.y.toFixed(2)}% (${ctx.raw.classification})` } } },
            scales: {
                x: { display: custom.showX, title: { display: true, text: 'Genes' }, ticks: { stepSize: 1, maxRotation: 45, minRotation: 45, callback: (val) => geneLabels[val] || '' } },
                y: { display: custom.showY, title: { display: true, text: 'Mean % Ciliated' } }
            }
        }
    });
}
...
// =============================================================================
// MAIN ORCHESTRATOR & OTHER FUNCTIONS
// =============================================================================
...
        return;
    }
    const { foundGenes } = findGenes([...new Set(originalQueries.map(q => q.trim()))]);
...
    const plotFunctions = {
        localization_bubble: renderBubblePlot,
        functional_bar: renderBarPlot,
        enrichment_bubble: renderEnrichmentBubblePlot,
        balloon_plot: renderBalloonPlot,
        venn_diagram: renderVennDiagram,
        network: (g, c) => renderComplexNetwork(g, plotContainer, c),
        organelle_radar: (g, c) => renderOrganelleRadarPlot(g, plotContainer, c),
        organelle_umap: (g, c) => renderOrganelleUMAP(g, plotContainer, c),
        screen_analysis: (g, c) => renderGeneScreenAnalysis(g, plotContainer, c),
        expression_heatmap: (g, c) => renderExpressionHeatmap(plotExpressionData, g)
    };
    if (plotFunctions[plotType]) {
        plotFunctions[plotType](foundGenes, custom);
    } else {
